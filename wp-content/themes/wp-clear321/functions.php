...
// Add Wide Narrow Content before opening #content div tag
add_action('solostream_before_content', 'featured_narrow');

// Add Wide Featured Galleries before opening #content div tag
add_action('solostream_before_content', 'featured_galleries');

// Add Wide Featured Videos before opening #content div tag
add_action('solostream_before_content', 'featured_videos');
...
// Featured Galleries/Category
function featured_galleries() {
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
	global $wp_query;
	$postid = $wp_query->post->ID;
	if ( is_home() && $paged < 2 && get_option('solostream_galleries_on') == 'Yes' ) { 
		get_template_part( 'featured', 'galleries' );
	}
	if ( is_singular() && get_post_meta( $postid, 'post_featgalleries', true ) == "Yes" ) {
		get_template_part( 'featured', 'galleries' );
	}
}
...
function featured_videos() {
	...
	global $wp_query;
	$postid = $wp_query->post->ID;
	if ( is_home() && $paged < 2 && get_option('solostream_videos_on') == 'Yes' ) { 
		get_template_part( 'featured', 'vids' );
	}
	if ( is_singular() && get_post_meta( $postid, 'post_featvideo', true ) == "Yes" ) {
		get_template_part( 'featured', 'vids' );
	}
}
